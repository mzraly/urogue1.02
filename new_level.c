...
#include "rogue.h"
 ...
 /* ltype = designates type of level to create */
 {
     ...
     } while (CCHAR( mvinch(stairs.y, stairs.x) ) != FLOOR && cnt++ <= 5000);
     addch(STAIRS);
     ...
     if (levtype == NORMLEV) {
        if (rnd(10) < level) {
            ...
            while (i--)
            {
                ...
                switch(rnd(11)) {
                    case 0: ch = TRAPDOOR;
                    break; case 1: ch = BEARTRAP;
                    break; case 2: ch = SLEEPTRAP;
                    break; case 3: ch = ARROWTRAP;
                    break; case 4: ch = TELTRAP;
                    break; case 5: ch = DARTTRAP;
                    break; case 6: ch = POOL;
                            traps[i].tr_flags = ISFOUND;
                    break; case 7: ch = MAZETRAP;
                    break; case 8: ch = FIRETRAP;
                    break; case 9: ch = POISONTRAP;
                    break; case 10: ch = RUSTTRAP;
                }
                addch(ch);
                traps[i].tr_type = ch;
                traps[i].tr_show = FLOOR;
 ...
 /* ltype = designates type of level to create */
 {
     ...
     if (ltype != MAZELEV && rnd(30) < level - 14) {
        ...
        do {
            ...
            width = rp->r_max.y - 2;
            length = rp->r_max.x - 2;
        } while ((width*length > MAXTREAS) && (i++ <= MAXROOMS*4));
