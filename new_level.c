 ...
 /* ltype = designates type of level to create */
{
	...
	do {
		...
		continue;
	    rnd_pos(&rooms[rm], &stairs);
	} while (CCHAR(mvinch(stairs.y, stairs.x)) != FLOOR
		 && cnt++ <= 5000);
	addch(POST);
    }
 ...
 /* ltype = designates type of level to create */
{
	...
	for (j = 1; j < rp->r_max.y - 1; j++)
		...
		if ((rnd(100) < (MAXTREAS * 100) / (width * length)) &&
		     ...
		     */
		    if (got_unique)
			new_monster(item, randmonster(FALSE, TRUE), &trp,
				    FALSE);
		    else {
			new_monster(item, randmonster(FALSE, FALSE), &trp,
				    FALSE);
			if (on(*tp, ISUNIQUE))
			    got_unique = TRUE;
		    }
		...
		/* Treasures */
		if ((rnd(100) < (MAXTREAS * 100) / (width * length)) &&
		    (CCHAR(mvinch(rp->r_pos.y + j, rp->r_pos.x + i)) ==
		     FLOOR)) {
		    item = new_thing();
		    attach(lvl_obj, item);
