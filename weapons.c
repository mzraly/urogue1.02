...
do_motion(struct object *obj, int ydelta, int xdelta, struct thing *tp)
{
	...
	obj->o_pos.y += ydelta;
	obj->o_pos.x += xdelta;
	if (shoot_ok(ch = winat(obj->o_pos.y, obj->o_pos.x)) && ch != DOOR
	    && !ce(obj->o_pos, hero)) {
	    /*
 ...
 */
int fall(struct linked_list *item, int pr)
{
    struct object *obj;
    struct room *rp;
    static coord fpos;
	...
	if (rp != NULL &&
	    (!(rp->r_flags & ISDARK) || (rp->r_flags & HASFIRE))) {
	    light(&hero);
	    mvwaddch(cw, hero.y, hero.x, PLAYER);
	}
	return ret;
