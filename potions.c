...
void quaff(int which, int blessed)
{
    struct object *obj;
	...
	if (cursed) {
		...
		fuse(unsee, 0, blessed ? SEEDURATION * 3 : SEEDURATION,
		     AFTER);
		light(&hero);
	...
	if (off(player, ISINVIS)) {
	    ...
	    lengthen(appear, blessed ? WANDERTIME * 3 : WANDERTIME);
	break;
    case P_SMELL:
	if (cursed) {
	...
	if (cursed) {
		...
		if (find_slot(unbhero))
		    ...
		    extinguish(unshero);
		    unshero();
		    fuse(unbhero, 0, 2 * (HEROTIME + rnd(HEROTIME)),
			 AFTER);
		}
	...
	p_guess[which] = NULL;
	       ...
	       !p_know[which] && askme && p_guess[which] == NULL) {
...
void res_wisdom(void)
{
    ...
    ring_str =
	ring_value(R_ADDWISDOM) + (on(player, POWERWISDOM) ? 10 : 0);
...
void add_const(int cursed)
{
    ...
    /* Adjust the maximum */
    if (max_stats.s_const <
	pstats.s_const - (on(player, POWERCONST) ? 10 : 0))
