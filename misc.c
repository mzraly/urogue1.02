...
char *tr_name(int ch)
{
	...
	break;
    case POISONTRAP:
	s = terse ? "A poison pool trap." :
	    "You found a poison pool trap.";
	break;
    case LAIR:
...
void look(int wakeup)
{
    ...
    int och;
    int oldx, oldy;
    int inpass, horiz, vert, do_light = FALSE, do_blank = FALSE;
    int passcount = 0;
    struct room *rp;
    int ey, ex;
	...
	for (y = player.t_oldpos.y - 1; y <= player.t_oldpos.y + 1; y++) {
	    ...
	    if (levtype == MAZELEV && (ch != '|' && ch != '-') &&	/* Not a wall */
	...
	if (x >= 0 && x < COLS)
		...
		if (isalpha(mvwinch(mw, y, x))) {
			...
			tp->t_oldch =
			    (trp->tr_flags & ISFOUND) ? tp->t_oldch : trp->
			    tr_show;
		    }
		...
		if (off(player, ISBLIND)) {
		    ...
		    if (levtype == MAZELEV && (rp != NULL) && (ch != '|' && ch != '-') &&	/* Not a wall */
...
{
    int start, goal, delta, ycheck = 0, xcheck = 0, absy, absx;
    int row;
