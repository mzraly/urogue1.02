...
void do_zap(int gotdir, int which, int blessed)
{
    ...
    case WS_CANCEL:
	{
		...
		if (which == WS_POLYMORPH) {
		    ...
		    delta.y = y;
		    delta.x = x;
		    new_monster(item, rnd(NUMMONST - 1) + 1, &delta,
				FALSE);
		    monster = tp->t_type;
		    if (off(*tp, ISRUN))
			runto(&delta, &hero);
			...
			tp->t_oldch =
			    CCHAR(mvwinch(cw, tp->t_pos.y, tp->t_pos.x));
		}
	    }
	}
	break;
...
int
shoot_bolt(struct thing *shooter, coord start, coord dir, int get_points,
	   int reason, char *name, int damage)
{
	...
	case ' ':
		...
		default:	/* A wall */
		    {
			int chy =
			    CCHAR(mvinch(pos.y - dir.y, pos.x + dir.x)),
			    chx =
			    CCHAR(mvinch(pos.y + dir.y, pos.x - dir.x));
	...
	default:
		...
		if (!save
			...
			damage = 0;
			if (rnd(100) < 75
			    && cur_weapon != NULL && shooter != &player) {
