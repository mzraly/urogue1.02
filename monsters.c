...
int randmonster(int wander, int no_unique)
{
     ...
     */
    if (wander && monsters[NUMMONST].m_wander && rnd(100) < 3)
	return NUMMONST;
	...
	d = NLEVMONS * (cur_level - 1) + (rnd(range) -
					  (range - 1 - NLEVMONS));
	if (d < 1)
	    d = rnd(NLEVMONS) + 1;
	if (d > NUMMONST - NUMUNIQUE - 1) {
		...
		d = rnd(range + NUMUNIQUE) + (NUMMONST - 1) - (range +
							       NUMUNIQUE -
							       1);
	}
    }
	    ...
	    int max_monster)
{
    ...
    tp->t_stats.s_exp =
	mp->m_stats.s_exp + mp->m_add_exp * tp->t_stats.s_hpt;
	...
	if ((cur->o_hplus <= 0) && (cur->o_dplus <= 0))
	...
	if (tp->t_pack != NULL)
		...
		break;
	    case 8:
		mch = monsters[randmonster(FALSE, FALSE)].m_appear;
		break;
	    case 9:
		mch = ARTIFACT;
	    }
...
struct linked_list *wake_monster(int y, int x)
{
	...
	/* Sleep */
	if (on(*tp, CANSNORE) && no_command == 0 && !save(VS_PARALYZATION)) {
    ...
    if (((on(*tp, ISINVIS) || (on(*tp, ISSHADOW) && rnd(100) < 90)) &&
	 off(player, CANSEE)) || on(*tp, ISINWALL) || on(*tp, CANSURPRISE))
	mvwinch(stdscr, y, x);
...
void genocide(void)
{
    ...
    int num_monst = NUMMONST - 1, pres_monst = 1, num_lines =
	2 * (LINES - 3);
    int which_monst;
    char monst_name[40];
	...
	if ((num_monst -= num_lines) > 0) {
	    ...
	    if (!terse)
		waddstr(hw, " do you wish to wipe out");
	    waddstr(hw, "? ");
	    draw(hw);
	}
    }
  ...
  get_monst:
	...
	mvwaddstr(hw, 0, 0,
		  "Please enter a number in the displayed range -- ");
	draw(hw);
	goto get_monst;
    }
...
int id_monst(int monster)
{
    ...
    for (; result > 0; result--)
	if (monsters[result].m_appear == monster)
	    return (result);
...
void check_residue(struct thing *tp)
{
    ...
    if (on(*tp, DIDDRAIN) && rnd(2) == 0)
	raise_level();

    /* If frightened of this monster, stop */
    if (on(player, ISFLEE) && player.t_dest == &tp->t_pos)
...
void sell(struct thing *tp)
{
    ...
    item = spec_item(goods, selection[which_item].which,
		     selection[which_item].plus1,
		     selection[which_item].plus2);
    ...
    switch (goods) {
    case STICK:
	ws_know[selection[which_item].which] = TRUE;
	break;
    case RING:
	r_know[selection[which_item].which] = TRUE;
    }
