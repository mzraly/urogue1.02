...
new_monster(struct linked_list *item, int type, coord *cp, int max_monster)
{
    ...
    if (on(*tp, ISDISGUISE))
    {
        ...
        if (tp->t_pack != NULL)
            ...
            {
                case 0: mch = GOLD;
                break; case 1: mch = POTION;
                break; case 2: mch = SCROLL;
                break; case 3: mch = FOOD;
                break; case 4: mch = WEAPON;
                break; case 5: mch = ARMOR;
                break; case 6: mch = RING;
                break; case 7: mch = STICK;
                break; case 8: mch = monsters[randmonster(FALSE, FALSE)].m_appear;
                break; case 9: mch = ARTIFACT;
            }
        tp->t_disguise = mch;
    }
}
...
wanderer(void)
{
    ...
    do {
        do {
            i = rnd_room();
        } while (hr == &rooms[i] && levtype != MAZELEV && levtype != THRONE);
        rnd_pos(&rooms[i], &cp);
...
sell(struct thing *tp)
{
    ...
    switch (goods) {
        ...
        break; case RING:  r_know[selection[which_item].which] = TRUE;
    }
