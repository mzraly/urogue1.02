...
void do_chase(struct thing *th, int flee)
{
    ...
    *new_room;			/* new room of monster */
    int mindist = MAXINT, maxdist = MININT, dist = MININT, i, last_door = -1;	/* Door we just came from */
	...
	if (th->t_doorgoal != -1) {	/* Do we already have the goal? */
		...
		if (mvwinch(stdscr, exity, exitx) == DOOR) {
			...
			dist =
			    DISTANCE(th->t_dest->y, th->t_dest->x, exity,
				     exitx);
			...
			if (flee)
			    dist -=
				DISTANCE(th->t_pos.y, th->t_pos.x, exity,
					 exitx);
	...
	if ((item = find_obj(ch_ret.y, ch_ret.x)) != NULL) {
	    int floor = (roomin(&ch_ret) == NULL) ? PASSAGE : FLOOR;
...
int chase(struct thing *tp, coord * ee, int flee, int *mdead)
{
	...
	*mdead = 0;
     ...
     */
    if ((on(*tp, ISHUH) && rnd(10) < 8) || ((on(*tp, ISINVIS) || on(*tp, ISSHADOW)) && rnd(100) < 20) || (on(player, ISINVIS) && off(*tp, CANSEE))) {	/* Player is invisible */
	/*
     ...
     * enough
     */
    else if ((off(*tp, ISFLEE)
	      || rnd(DISTANCE(er->y, er->x, ee->y, ee->x)) > 2)
	...
	for (x = er->x - 1; x <= ex; x++)
		...
		if (((off(*tp, ISFLEE) && !ce(hero, *ee)) ||
		     (on(player, ISDISGUISE)
		      && (rnd(tp->t_stats.s_lvl) < 6))) && x == hero.x
		    && y == hero.y)
		    continue;
    ...
    if (off(*tp, ISFLEE) &&
	((tp->t_dest != &hero) || off(player, ISINWALL)
	 || on(*tp, CANINWALL)))
	return (dist != 0);
...
int cansee(int y, int x)
{
     ...
     */
    return ((rer != NULL && rer == roomin(&hero) && (!(rer->r_flags & ISDARK) || (rer->r_flags & HASFIRE)) && (levtype != MAZELEV ||	/* Maze level needs direct line */
													       maze_view
													       (tp.
														y,
														tp.
...
int straight_shot(int ery, int erx, int eey, int eex, coord * shooting)
{
    ...
    if ((ery != eey) && (erx != eex) && (abs(ery - eey) != abs(erx - eex)))
	return (FALSE);
