...
apply(void)
{
        ...
        if (obj->art_stats.ar_flags & ISACTIVE) {
            switch (which) {
                ...
                break; case TR_PHIAL:
                    do_phial();
                break; case TR_AMULET:
                    do_amulet();
                break; case TR_PALANTIR:
                    do_palantir();
                break; case TR_CROWN:
                    do_crown();
                break; case TR_SCEPTRE:
                    do_sceptre();
                break; case TR_SILMARIL:
                    do_silmaril();
                break; case TR_WAND:
                    do_wand();
                    break;
...
do_palantir(void)
{
    ...
    switch (which) {
        ...
        break; case 1: read_scroll(S_GFIND, FALSE);
        break; case 2: quaff(P_TFIND, FALSE);
        break; case 3: read_scroll(S_FOODFIND, FALSE);
        break; case 4: read_scroll(S_TELEP, FALSE);
        break; case 5: quaff(P_CLEAR, FALSE);
            break;
        default:
                msg("What a strange thing to do!!");
    }
...
do_silmaril(void)
{
    ...
    switch (which) {
        ...
        break; case 1: read_scroll(S_PETRIFY, FALSE);
        break; case 2:
...
do_amulet(void)
{
    ...
    switch (which) {
        ...
        break; case 1: quaff(P_INVIS, FALSE);
            break;
        default:
                msg("What a strange thing to do!!");
    }
...
do_sceptre(void)
{
    ...
    switch (which) {
        ...
        break; case 1: if (get_dir())
                    do_zap(TRUE, WS_POLYMORPH, FALSE);
        break; case 2: if (get_dir())
                    do_zap(TRUE, WS_SLOW_M, FALSE);
        break; case 3: if (get_dir())
                    do_zap(TRUE, WS_TELMON, FALSE);
        break; case 4: if (get_dir())
                    do_zap(TRUE, WS_CONFMON, FALSE);
        break; case 5: if (get_dir())
                    do_zap(TRUE, WS_PARALYZE, FALSE);
        break; case 6: if (get_dir())
                    do_zap(TRUE, WS_DRAIN, FALSE);
        break; case 7: quaff(P_SMELL, FALSE);
            break;
        default:
                msg("What a strange thing to do!!");
    }
