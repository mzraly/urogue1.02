...
void picky_inven(void)
{
	...
	msg("%c) %s", pack->l_letter,
	    inv_name((struct object *) ldata(pack), FALSE));
    else {
...
struct linked_list *get_item(char *purpose, int type)
{
	...
	if (type != 0 && type != CALLABLE && type != MARKABLE) {
		...
		do {
			...
			if (type == STICK && opb->o_type == WEAPON) {
			    msg("You seem to have nothing to %s.",
				purpose);
			    after = FALSE;
			    return NULL;
			}
			mpos = 0;
			msg("%c) %s", savepit->l_letter,
			    inv_name(opb, FALSE));
			continue;
		    }
		    if (och == ESCAPE) {
	...
	for (;;) {
		...
		struct object *opb;
		opb = OBJPTR(obj);
		if ((type == WEAPON || type == 0 || type == MARKABLE
		     || type == CALLABLE)
