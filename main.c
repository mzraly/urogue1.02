...
int main(int argc, char *argv[])
{
	...
	strcpy(fruit,
	       funfruit[rnd(sizeof(funfruit) / sizeof(funfruit[0]))]);
    }
	...
	if (strcmp
	    (PASSWD,
	     md_crypt(md_getpass("Wizard's password: "),
		      PASSWDSEED)) == 0) {
	    if (canwizard) {
#ifdef SIGQUIT
    ...
    if (too_much() && !wizard) {
	if (!author()) {
	    static char *niceties[] =
		{ "juicy", "fresh", "nice", "sweet" };
