...
void do_move(int dy, int dx)
{
	...
	if (rnd(10) < 1) {
	    ...
	    msg("You regain your composure.");
		   ...
		   < DISTANCE(hero.y, hero.x, player.t_dest->y,
			      player.t_dest->x))
	    return;
    }
...
int be_trapped(struct thing *th, coord * tc)
{
    struct trap *tp;
    char *mname = NULL;
    int ch, is_player = (th == &player), can_see = cansee(tc->y, tc->x);
    ...
    case ARROWTRAP:
	if (swing
	    (th->t_ctype, th->t_stats.s_lvl - 1, th->t_stats.s_arm, 1)) {
	    if (is_player) {
    ...
    case POOL:{
		...
		if (i < 30) {
		    if (can_see)
			msg("The %s drowned in the pool!", mname);
		    killed(mitem, FALSE, FALSE);
		}
	    }
	}
    ...
    case FIRETRAP:{
		...
		if (on(*th, CANBBURN)) {
		    if (can_see)
			msg("The %s is burned to death by the flames.",
			    mname);
		    check_residue(th);
		    remove_monster(&th->t_pos, mitem);
		} else if (on(*th, NOFIRE)) {
		    if (can_see)
			msg("The %s passes through the flames unharmed.",
			    mname);
		} else {
		    if (can_see)
			...
			if (can_see)
			    msg("The %s turns and runs away in fear.",
				mname);
			turn_on(*th, ISFLEE);
		    }
		}
...
void set_trap(struct thing *tp, int y, int x)
{
	...
	|| levtype == POSTLEV
	|| rnd(60) >=
	(tp->t_stats.s_dext + tp->t_stats.s_lvl / 2 + thief_bonus)) {
	if (is_player)
	    msg("The trap failed!");
	return;
    }
