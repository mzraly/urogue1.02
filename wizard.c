...
create_obj(int which_item, int which_type, int cursed)
{
    ...
    switch(newitem) {
        ...
        break; case SCROLL:     whc = TYP_SCROLL;       tsize = msz = MAXSCROLLS;
        break; case FOOD:       whc = TYP_FOOD;         tsize = msz = MAXFOODS;
        break; case WEAPON:     whc = TYP_WEAPON;       tsize = msz = MAXWEAPONS;
        break; case ARMOR:      whc = TYP_ARMOR;        tsize = msz = MAXARMORS;
        break; case RING:       whc = TYP_RING;         tsize = msz = MAXRINGS;
        break; case STICK:      whc = TYP_STICK;        tsize = msz = MAXSTICKS;
        break; case ARTIFACT:   whc = TYP_ARTIFACT;     tsize = msz = MAXARTIFACT;
        break; default:
            if (thiswin == hw)
                restscr(cw);
            mpos = 0;
    ...
    if(msz == 1) {              /* if only one type of item */
        ...
        switch(newitem) {
                ...
                break; case FOOD:       wmi = &fd_data[0];
                break; case ARTIFACT:   war = &arts[0];
                break; case WEAPON:     wmn = 1;
                break; case ARMOR:      wmn = 2;
        }
        wclear(hw);
        thiswin = hw;
        while (msz > 0) {
...
makemon(void)
{
    ...
    } while (which_monst <= 0 || which_monst > NUMMONST+1);
    restscr(cw);
    ...
    if (ISWEARING(R_TELCONTROL)) {
        ...
        while (which != ESCAPE
            ...
            switch(which) {
                ...
                break; case 'k': c.y--;
                break; case 'l': c.x++;
                break; case 'y': c.x--; c.y--;
                break; case 'u': c.x++; c.y--;
                break; case 'b': c.x--; c.y++;
                break; case 'n': c.x++; c.y++;
                break; case '*':
