...
void create_obj(int which_item, int which_type, int cursed)
{
	...
	wprintw(hw, "%s\t %c\n%s\t %c\n", things[TYP_POTION].mi_name,
		POTION, things[TYP_SCROLL].mi_name, SCROLL);
    ...
    case POTION:
	whc = TYP_POTION;
	tsize = msz = MAXPOTIONS;
	break;
    case SCROLL:
	whc = TYP_SCROLL;
	tsize = msz = MAXSCROLLS;
	break;
    case FOOD:
	whc = TYP_FOOD;
	tsize = msz = MAXFOODS;
...
int teleport(void)
{
	...
	while (which != ESCAPE
	       && which != LINEFEED && which != CARRIAGE_RETURN) {
	    switch (which) {
	    case 'h':
		c.x--;
		break;
