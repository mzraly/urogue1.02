 ...
 */
int fight(coord * mp, struct object *weap, int thrown)
{
    struct thing *tp;
	...
	did_hit = FALSE;
	mname =
	    (on(player, ISBLIND)) ? "it" : monsters[tp->t_index].m_name;
	if (!can_blink(tp)
	    && (off(*tp, MAGICHIT)
		|| (weap != NULL
		...
		if (on(player, CANHEAR)) {
		    ...
		    msg("The %s seems to be trying to make some noise.",
			mname);
		aggravate();
...
int attack(struct thing *mp, struct object *weapon, int thrown)
{
    ...
    mname =
	on(player, ISBLIND) ? "the monster" : monsters[mp->t_index].m_name;
	...
	if (!thrown) {
		...
		if (!save(VS_POISON)) {
			...
			msg("You pass out from the stench of the %s.",
			    mname);
			no_command += 4 + rnd(8);
	     ...
	     */
	    if ((on(*mp, CANDRAIN) || on(*mp, DOUBLEDRAIN))
		&& rnd(100) < 15) {
		...
		if (steal != NULL) {
			...
			*op = *obj;
			...
			turn_on(*mp, NOMOVE);
			msg("The %s is transfixed by your ownership spell.", mname);
		    }
		    if (rnd(2))
			turn_on(*mp, ISFLEE);
		    turn_on(*mp, ISINVIS);
...
int swing(int class, int at_lvl, int op_arm, int wplus)
{
	...
	((min(at_lvl, att_mat[class].max_lvl) -
	  att_mat[class].offset) / att_mat[class].range) + (10 - op_arm);
    if (need > 20 && need <= 25)
	need = 20;
...
roll_em(struct thing *att_er, struct thing *def_er, struct object *weap,
	int hurl, struct object *cur_weapon)
{
	...
	if ((weap->o_flags & ISMISL) && cur_weapon != NULL &&
	    ...
	    cp = (weap->o_flags & ISMISL ? weap->o_damage : weap->
		  o_hurldmg);
    else {
	cp = weap->o_damage;
	/*
	...
	if ((weap != NULL && weap->o_type == WEAPON &&
		...
		debug("Damage for %dd%d came out %d.", ndice, nsides,
		      proll);
...
void thunk(struct object *weap, char *mname)
{
	...
	return;
    if (weap != NULL && weap->o_type == WEAPON)
	msg("The %s hits %s.", weaps[weap->o_which].w_name,
	    prname(mname, FALSE));
    else
...
void m_thunk(struct object *weap, char *mname)
{
	...
	return;
    if (weap != NULL && weap->o_type == WEAPON)
	msg("%s's %s hits you.", prname(mname, TRUE),
	    weaps[weap->o_which].w_name);
    else
...
void bounce(struct object *weap, char *mname)
{
	...
	return;
    if (weap != NULL && weap->o_type == WEAPON)
	msg("The %s misses %s.", weaps[weap->o_which].w_name,
	    prname(mname, FALSE));
    else
...
void m_bounce(struct object *weap, char *mname)
{
	...
	return;
    if (weap != NULL && weap->o_type == WEAPON)
	msg("%s's %s misses you.", prname(mname, TRUE),
	    weaps[weap->o_which].w_name);
    else
